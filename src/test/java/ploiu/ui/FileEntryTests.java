...
class FileEntryTests {
    ...
    void start(Stage stage) {
        ...
        tags = Set.of(new TagApi(0L, "tag1", null), new TagApi(1L, "tag2", null), new TagApi(2L, "tag3", null));
        var file = new FileApi(0, "name.txt", tags, 0L, null, null, "unknown");
    ...
    void testRenameFileConfirm(FxRobot robot) {
        ...
        verify(fileReceiver).process(argThat(it -> {
            if (it instanceof FileUpdateEvent evt) {
                // I mean...it doesn't MATTER what we pass for size/dateCreated/fileType...so it might be ok to just check the specific fields instead of including those 3
